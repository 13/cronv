...
use crate::cron::{CronEntry, CronSchedule, CrontabLine, parse_crontab, serialize_crontab};
...
impl EditForm {
    ...
    pub fn prev_field(&mut self) {
        self.focused = if self.is_special {
            match self.focused {
                FormField::Command => FormField::Special,
                _ => FormField::Command,
            }
        } else {
            match self.focused {
    ...
    pub fn to_entry(&self, enabled: bool) -> CronEntry {
        CronEntry {
            ...
            command: self.command.value.trim().into(),
        }
    }
...
impl App {
    pub fn new(source: CrontabSource) -> Result<Self> {
        ...
        Ok(App {
            ...
            source,
            use_24h: true,
            raw_edit_requested: false,
            table_top_row: 5,
            modal_bounds: None,
            comment_input_bounds: None,
            edit_click_targets: Vec::new(),
            last_click_row: None,
            last_click_at: None,
        })
    }
    ...
    pub fn entry_count(&self) -> usize {
        self.lines
            .iter()
            .filter(|l| matches!(l, CrontabLine::Entry(_)))
            .count()
    }
        ...
        self.lines
            ...
            .filter_map(|l| {
                if let CrontabLine::Entry(e) = l {
                    if e.enabled { Some(&e.schedule) } else { None }
                } else {
                    None
                }
            })
    ...
    pub fn move_row_up(&mut self) {
        ...
        let a = match rows[self.selected - 1] {
            VisibleRow::Entry(i) | VisibleRow::Comment(i) => i,
        };
        let b = match rows[self.selected] {
            VisibleRow::Entry(i) | VisibleRow::Comment(i) => i,
        };
        self.lines.swap(a, b);
        self.selected -= 1;
    ...
    pub fn move_row_down(&mut self) {
        ...
        let a = match rows[self.selected] {
            VisibleRow::Entry(i) | VisibleRow::Comment(i) => i,
        };
        let b = match rows[self.selected + 1] {
            VisibleRow::Entry(i) | VisibleRow::Comment(i) => i,
        };
        self.lines.swap(a, b);
        self.selected += 1;
    ...
    pub fn start_add(&mut self) {
        ...
        let insert_after = self.visible_rows().get(self.selected).map(|r| match r {
            VisibleRow::Entry(i) | VisibleRow::Comment(i) => *i,
        });
    ...
    pub fn confirm_edit(&mut self) {
        ...
        let enabled = form
            .editing_index
            .and_then(|i| {
                if let CrontabLine::Entry(e) = &self.lines[i] {
                    Some(e.enabled)
                } else {
                    None
                }
        ...
        if let Some(li) = form.editing_index {
            ...
            match form.insert_after {
                Some(li) => {
                    self.lines.insert(li + 1, CrontabLine::Entry(new_entry));
                }
                None => {
            ...
            match target {
                ...
                CommentTarget::InsertAfter(li) => {
                    if !new_text.is_empty() {
                        self.lines
                            .insert(li + 1, CrontabLine::Comment(format!("# {}", new_text)));
                ...
                CommentTarget::Append => {
                    if !new_text.is_empty() {
                        self.lines
                            .push(CrontabLine::Comment(format!("# {}", new_text)));
        ...
        match key.code {
            ...
            KeyCode::Enter => {
                let on_cmd = self
                    .form
                    .as_ref()
                    .map(|f| f.focused == FormField::Command)
                    .unwrap_or(false);
                if on_cmd {
                    self.confirm_edit();
            ...
            KeyCode::Left => {
                if let Some(f) = &mut self.form {
                    f.active_input_mut().move_left();
                }
            }
            KeyCode::Right => {
                if let Some(f) = &mut self.form {
                    f.active_input_mut().move_right();
                }
            }
            KeyCode::Home => {
                if let Some(f) = &mut self.form {
                    f.active_input_mut().home();
                }
            }
            KeyCode::End => {
                if let Some(f) = &mut self.form {
                    f.active_input_mut().end();
                }
            }
            KeyCode::Backspace => {
                if let Some(f) = &mut self.form {
                    f.active_input_mut().backspace();
                }
            }
            KeyCode::Delete => {
                if let Some(f) = &mut self.form {
                    f.active_input_mut().delete_char();
                }
            }
            ...
            KeyCode::Char(c) if !ctrl => {
                if let Some(f) = &mut self.form {
                    f.active_input_mut().insert(c);
                }
            }
            KeyCode::F(1) => {
                if let Some(f) = &mut self.form {
                    f.is_special = !f.is_special;
                    f.focused = if f.is_special {
                        FormField::Special
                    } else {
                        FormField::Minute
                    };
    ...
    fn key_comment(&mut self, key: KeyEvent) -> Result<bool> {
        ...
        match key.code {
            ...
            KeyCode::Left => {
                if let Some((inp, _)) = &mut self.comment_input {
                    inp.move_left();
                }
            }
            KeyCode::Right => {
    ...
    match source {
        ...
        CrontabSource::System => {
            ...
            if err.contains("no crontab") {
                ...
                Err(anyhow::anyhow!("crontab -l: {}", err.trim()))
            }
        }
    }
}
