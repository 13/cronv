...
pub enum CronSchedule {
    Standard { minute: String, hour: String, day: String, month: String, weekday: String },
    Special(String),
}
    ...
    pub fn next_n_runs(&self, n: usize, use_24h: bool) -> Vec<(NaiveDateTime, String)> {
        ...
        for _ in 0..n {
            match self.next_after(from) {
                Some(t) => { results.push((t, format_abs(t, use_24h))); from = t; }
                None    => break,
            }
        }
        results
    ...
    pub fn firings_per_hour(&self) -> [u8; 24] {
        ...
        match self {
            CronSchedule::Special(s) => match s.to_lowercase().as_str() {
                ...
                "@daily" | "@midnight"  => { counts[0] = 1; }
                "@weekly"               => { counts[0] = 1; }
                "@monthly"              => { counts[0] = 1; }
                "@yearly" | "@annually" => { counts[0] = 1; }
                _                       => {}
            },
...
fn describe_standard(
    minute: &str, hour: &str, day: &str, month: &str, weekday: &str, use_24h: bool,
) -> String {
    if [minute, hour, day, month, weekday].iter().all(|f| *f == "*") {
        return "Every minute".into();
    }

    let mins = match expand(minute, 0, 59) { Some(v) => v, None => return "Invalid schedule".into() };
    let hrs  = match expand(hour,   0, 23) { Some(v) => v, None => return "Invalid schedule".into() };
    ...
    if day == "*" && month == "*" && weekday == "*" && hour == "*" {
        ...
        if let Some(n) = step_n(minute) {
            return if n <= 1 { "Every minute".into() } else { format!("Every {} minutes", n) };
        }
        // M * * * * — specific minute, every hour
        if mins == [0] { return "Every hour".into(); }
...
fn build_time_component(
    minute: &str, hour: &str, mins: &[u8], hrs: &[u8], use_24h: bool,
) -> Option<String> {
    let all_hrs = hour == "*";
    ...
    if let Some(n) = step {
        if all_hrs {
            ...
            return if n <= 1 { None } // handled at caller as "Every minute"
                   else       { Some(format!("every {} minutes", n)) };
        }
        // */N H,H,H ... → windows per hour
        let last_min: u8 = (59 / n) * n;
        let windows: Vec<String> = hrs.iter().map(|&h| {
            format!("{} to {}", fmt_time(h, 0, use_24h), fmt_time(h, last_min, use_24h))
        }).collect();
        return Some(format!("every {} minute{} from {}",
            n, if n == 1 { "" } else { "s" }, join_and(&windows)));
    }

    // Not a step — specific minutes
    if all_hrs {
        // specific minute, every hour
        if mins == [0] { return Some("every hour".into()); }
        let strs: Vec<String> = mins.iter().map(|&m| format!(":{:02}", m)).collect();
        return Some(format!("at {} past every hour", join_and(&strs)));
    }

    // Specific minutes + specific hours → enumerate all fire times
    let times: Vec<String> = hrs.iter().flat_map(|&h| {
        mins.iter().map(move |&m| fmt_time(h, m, use_24h))
    }).collect();

    if times.len() <= 8 { Some(join_and(&times)) } else { Some(format!("{} times per day", times.len())) }
}

// ── Month description ─────────────────────────────────────────────────────────

fn describe_months(mv: &[u8]) -> String {
    if mv.len() == 1 { return month_name(mv[0]); }
    // Contiguous range?
    if mv.windows(2).all(|w| w[1] == w[0] + 1) {
        return format!("{} through {}", month_name(mv[0]), month_name(*mv.last().unwrap()));
    }
...
const WD_PLURAL:   [&str; 7] = ["Sundays","Mondays","Tuesdays","Wednesdays","Thursdays","Fridays","Saturdays"];
const WD_SINGULAR: [&str; 7] = ["Sunday","Monday","Tuesday","Wednesday","Thursday","Friday","Saturday"];
...
fn describe_weekdays(s: &str) -> String {
    ...
    if let Some((a, b)) = s.split_once('-') {
        if let (Some(si), Some(ei)) = (parse_wd(a), parse_wd(b)) {
            if si == 1 && ei == 5 { return "Weekdays".into(); }
            if si == 0 && ei == 6 { return "Every day".into(); }
    ...
    if s.contains(',') {
        let names: Vec<&str> = s.split(',')
            ...
            .map(|i| WD_PLURAL[i])
            .collect();
        if !names.is_empty() { return join_and_strs(&names); }
    }
    // Single
    if let Some(i) = parse_wd(s) { return WD_PLURAL[i].into(); }
    s.into()
}

fn parse_wd(s: &str) -> Option<usize> {
    let mut n: usize = s.parse().ok()
        .or_else(|| WD_ABBREV.iter().position(|&a| a == s.to_lowercase().as_str()))?;
    if n == 7 { n = 0; }
...
fn describe_dom(s: &str) -> String {
    if let Ok(n) = s.parse::<u32>() { return format!("the {}", ordinal(n)); }
    if let Some((a, b)) = s.split_once('-') {
        if let (Ok(a), Ok(b)) = (a.parse::<u32>(), b.parse::<u32>()) {
            return format!("days {} to {}", a, b);
        }
    }
    if s == "L" { return "the last day".into(); }
    s.into()
}
...
pub fn fmt_time(h: u8, m: u8, use_24h: bool) -> String {
    if use_24h {
        ...
        let dh: u8 = match h { 0 => 12, 13..=23 => h - 12, _ => h };
        if m == 0 { format!("{} {}", dh, period) } else { format!("{}:{:02} {}", dh, m, period) }
    }
}
...
fn month_name(n: u8) -> String {
    const N: [&str; 13] = ["","January","February","March","April","May","June",
...
fn format_next(now: NaiveDateTime, next: NaiveDateTime, use_24h: bool) -> String {
    ...
    let rel = if mins < 60 {
        ...
        let m = mins - hours * 60;
        if m == 0 { format!("in {}h", hours) } else { format!("in {}h {}m", hours, m) }
    } else if days < 7 {
    ...
    let h = next.hour() as u8; let m = next.minute() as u8;
    let time_s = fmt_time(h, m, use_24h);
    if now.date() == next.date() {
        format!("{} ({})", time_s, rel)
    } else if days < 7 {
        let abbr = ["Sun","Mon","Tue","Wed","Thu","Fri","Sat"][next.weekday().num_days_from_sunday() as usize];
...
fn next_special(s: &str, now: NaiveDateTime) -> Option<NaiveDateTime> {
    match s.to_lowercase().as_str() {
        ...
        "@hourly"               => {
            ...
            if t > s { Some(t) } else { Some(t + Duration::hours(1)) }
        }
        _ => None,
    }
}
...
pub(crate) fn next_standard(
    ...
    while t < limit {
        ...
        if !months.contains(&mon) { t = advance_to_month(t, &months)?; continue; }
...
pub fn expand(expr: &str, lo: u8, hi: u8) -> Option<Vec<u8>> {
    ...
    for part in expr.split(',') {
        let part = part.trim();
        if part == "*" { set.extend(lo..=hi); continue; }
        if let Some(step_s) = part.strip_prefix("*/") {
            let step: u8 = step_s.parse().ok()?;
            if step == 0 { return None; }
            let mut v = lo;
            while v <= hi { set.push(v); v = v.saturating_add(step); }
            continue;
        }
        if let Some(dash) = part.find('-') {
            ...
            if b == 7 && hi == 6 { b = 6; }
            if a > hi || b > hi || a > b { return None; }
            let mut v = a;
            while v <= b { set.push(v); v = v.saturating_add(step); }
            continue;
        }
        let mut n: u8 = part.parse().ok()?;
        if n == 7 && hi == 6 { n = 0; } // Sunday alias
        if n < lo || n > hi { return None; }
        set.push(n);
    }
    set.sort_unstable(); set.dedup();
    if set.is_empty() { None } else { Some(set) }
}

fn field_valid(s: &str, lo: u8, hi: u8) -> bool { expand(s, lo, hi).is_some() }
    ...
    pub fn default_new() -> Self {
        CronEntry {
            ...
            schedule: CronSchedule::Standard {
                minute: "0".into(), hour: "9".into(),
                day: "*".into(), month: "*".into(), weekday: "*".into(),
            },
            command: String::new(),
        }
    }
...
pub fn parse_crontab(content: &str) -> Vec<CrontabLine> { content.lines().map(parse_line).collect() }

fn parse_line(raw: &str) -> CrontabLine {
    if raw.trim().is_empty() { return CrontabLine::Blank; }
    if raw.trim_start().starts_with('#') {
        let inner = raw.trim_start().trim_start_matches('#').trim();
        if let Some(e) = try_parse_entry(inner) {
            return CrontabLine::Entry(CronEntry { enabled: false, ..e });
        }
    ...
    if let Some(eq) = raw.find('=') {
        ...
        if !key.is_empty() && key.chars().all(|c| c.is_alphanumeric() || c == '_') {
            return CrontabLine::Variable { key: key.into(), value: raw[eq+1..].into() };
        }
    }
    if let Some(e) = try_parse_entry(raw) { return CrontabLine::Entry(e); }
...
fn try_parse_entry(s: &str) -> Option<CronEntry> {
    ...
    if s.starts_with('@') {
        ...
        return Some(CronEntry { enabled: true, schedule: CronSchedule::Special(kw.into()), command: cmd.into() });
    }
    ...
    if !field_valid(&min, 0, 59) { return None; }
    if !field_valid(&hr,  0, 23) { return None; }
    if !field_valid(&dom, 1, 31) { return None; }
    if !field_valid(&mon, 1, 12) { return None; }
    if !field_valid(&dow, 0,  6) { return None; }
    Some(CronEntry {
        enabled:  true,
        schedule: CronSchedule::Standard { minute: min, hour: hr, day: dom, month: mon, weekday: dow },
        command:  cmd,
    })
}
...
pub fn serialize_crontab(lines: &[CrontabLine]) -> String {
    ...
    for line in lines {
        match line {
            ...
            CrontabLine::Comment(s)              => { out.push_str(s); out.push('\n'); }
            CrontabLine::Variable { key, value } => { out.push_str(&format!("{}={}\n", key, value)); }
            CrontabLine::Entry(e) => {
                if !e.enabled { out.push_str("# "); }
...
pub const SPECIALS: &[SpecialEntry] = &[
    ...
    SpecialEntry { keyword: "@annually", desc: "Same as @yearly"                 },
    SpecialEntry { keyword: "@monthly",  desc: "1st of month at midnight"        },
    SpecialEntry { keyword: "@weekly",   desc: "Sunday at midnight"              },
    SpecialEntry { keyword: "@daily",    desc: "Daily at midnight"               },
    SpecialEntry { keyword: "@midnight", desc: "Same as @daily"                  },
    SpecialEntry { keyword: "@hourly",   desc: "Every hour at :00"               },
];
